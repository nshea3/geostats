...
sim_lq = simpgs(dbout=outdb, dbin=coal_db, rule=rule,props=c(0.5, 0.5), model1=gaus, neigh = voisin)
coal_db
plot(sim_lq)
plot(coal_db, add = T)
```
...
sim_a0 =  simtub(dbin = coal_a0,dbout=outdb,model=gaus_a0,neigh = neighb,mean=5000)

plot(sim_a0, pos.legend=2)
plot(coal_a0, add=T)
```
```{r}
...
sim_a1 =  simtub(dbin = coal_a1,dbout=outdb,model=gaus_a1,neigh = neighb,mean=15000)

plot(sim_a1,pos.legend=2)
plot(coal_a1,add=T)
```
***QUESTION***: 
...
Trying to use http://rgeostats.free.fr/html/db.selgrid.html to transfer the selection from one grid to the other. 

May have to work backwards and add a grid to the original DB? I don't know! Look at some more examples.

```{r}
sim_lq0_sel = db.sel(sim_lq, SimPGS.LQ.SFac1==1)
sim_lq1_sel = db.sel(sim_lq, SimPGS.LQ.SFac1==2)
...
```{r}
apwdb = db.create(pw_map,x0=c(0,0),dx=c(1,1),nx=c(200,100))
plot(db.locate(apwdb, 4, "z"), pos.legend = 3)
```
...
```
```{r}
selectivity(panel_q, cuts = seq(1000,2500,50))
```
