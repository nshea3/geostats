...
sim_thickness <- function(indb){
  ...
  simu <- simtub(nbtuba = 450, dbin=indb,dbout=outdb,model=Tmodel,neigh =     neighb,mean = c(10))
  return(simu)
...
sim_lq <- function(indb){
  ...
  sim_lq = simpgs(dbout=outdb, dbin=indb, rule=rule,props=c(0.5, 0.5), model1=gaus, neigh = voisin)
...
sim_a0 <- function(indb){
  ...
  outdb = db.create(x0=c(0,0),dx=c(1,1),nx=c(200,100))
  neighb = neigh.create(ndim = 2, type = 0)
  sim =  simtub(dbin = coal_a0,dbout=outdb,model=gaus_a0,neigh = neighb,mean=5000)
  return(sim)
